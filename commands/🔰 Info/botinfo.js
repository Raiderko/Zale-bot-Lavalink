  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    let tempmsg = await message.reply({
      embeds: [new Discord.MessageEmbed().setColor(es.color)
        .setAuthor(client.la[ls].cmds.info.botinfo.loading, "https://imgur.com/2BEcKBb.gif")
      ]
    })
    cpuStat.usagePercent(function(e, percent, seconds) {
      ...
      const botinfo = new Discord.MessageEmbed()
        ...
        .setColor(es.color).setThumbnail(`https://imgur.com/2BEcKBb.gif`)
        ...
        .addField(client.la[ls].cmds.info.botinfo.field3.title, 
          `\`\`\`CPU usage: ${percent.toFixed(2)}%,\nRAM usage: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}%\`\`\``
        )
        .addField(client.la[ls].cmds.info.botinfo.field4.title, `\`\`\`yml\nName: Raider\`\`\``, true)
        .addField(client.la[ls].cmds.info.botinfo.field5.title, handlemsg(`[**Invite Me**](https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands)`, {
        ...
        .setFooter(es.footertext, `https://imgur.com/2BEcKBb.gif`);
      tempmsg.edit({
        embeds: [botinfo]
      });
    });
